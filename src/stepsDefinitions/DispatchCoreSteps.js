...
const wmIntegration = dispatch.WorkMarketAssignment
...
export default class DispatchCore {
  ...
  async createProjectJobs (projectName: string = 'This is just a test project automation title') {
    await step('Tap Manage Project', async function () {
      await mainPage.tapManageProjects()
      await project.isLoaded()
    })

    await step('Search Project and tap Project Jobs tab', async function () {
      await project.searchProject(projectName)
      await project.tapProjecJobsTab()
    })

    await step('Add Project Jobs and Select Project Jobs Location', async function () {
      await project.tapAddProjecJobButton()
      await project.selectProjectLocation(2)
      assert.isTrue(await addproject.isSuccessMessageDisplayed(), 'Message success was not displayed')
      await addproject.tapOKButton()
    })
  }

  async searchTicket (dispathTitle: string) {
    await step('Search for Dispatch ticket', async function () {
      await dispatch.searchDispatch(dispathTitle)
    })
  }

  async createAssignMentForWorkMarket () {
    await step('Assigning Dispatch to Work Market Integration', async function () {
      await dispatch.tapAssignedVendorButton()
      await dispatch.tapCreateAssignmentButton()
      await wmIntegration.isLoaded()
      await wmIntegration.tapSchedule()
    })
  }
}
