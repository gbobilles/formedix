...
describe(`Client Portal - Creating a new dispatch ticket from client portal thru Bulk Upload - ${process.env.TEST_ENVIRONMENT.toUpperCase()} server`, function () {
  ...
  it(`Client Portal - Creating a new dispatch ticket from client portal - ${process.env.TEST_ENVIRONMENT.toUpperCase()} server`, async function () {
    ...
    await clientPortal.clientBulkUpload()
